    ...
    <form onSubmit={handleSubmit(onSubmit)} className="max-w-md mx-auto p-6 bg-white rounded shadow mt-10">
      ...
      <div className="mb-4">
        ...
        <div className="space-y-2">
          {["manage_patients", "manage_appointments", "manage_schedule"].map((permission) => (
            <label key={permission} className="flex items-center gap-2">
              <input type="checkbox" value={permission} {...register("permissions")} />
      ...
      <button
        type="submit"
        disabled={loading}
        className={`w-full py-2 rounded coursor-pointer ${loading ? "bg-gray-400" : "bg-green-600 hover:bg-green-700 text-white"}`}
      >
